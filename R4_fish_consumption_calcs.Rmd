...
"Caught_Rainbow_Trout,_servings_eaten","Past_catches:_Rainbow_Trout,_servings_eaten",                     
...
"Caught_Catfish,_servings_eaten","Past_catches:_Catfish,_servings_eaten",                     
...
check3 <-fisherdb %>% select(
  ...
  "Caught_Bluegill,_servings_eaten","Past_catches:_Blue_gill,_servings_eaten",                     
...
check4 <-fisherdb %>% select(
  ...
  "Caught_Common_Carp,_servings_eaten","Past_catches:_Common_Carp,_servings_eaten",                     
...
check5 <-fisherdb %>% select(
  ...
  "Caught_Bass,_servings_eaten","Past_catches:_Bass,_servings_eaten",                     
...
check6 <-fisherdb %>% select(
  ...
  "Caught_Crappie,_servings_eaten","Past_catches:_Crappie,_servings_eaten",                     
...
check7 <-fisherdb %>% select(
  ...
  "Caught_Tilapia,_servings_eaten","Past_catches:_Tilapia,_servings_eaten",                     
...
check8 <-fisherdb %>% select(
  ...
  "Caught_Other,_servings_eaten","Past_catches:_other,_servings_eaten",                     
...
#   `Past_catches:_Rainbow_Trout,_times_eaten_numeric`,`Past_catches:_Rainbow_Trout,_servings_eaten_numeric`,rainbow_trout_g_day,
#   catfish_g_day,`Past_catches:_Catfish,_times_eaten_numeric`,`Past_catches:_Catfish,_servings_eaten_numeric`,
#   bluegill_g_day,`Past_catches:_Bluegill,_times_eaten_numeric`,`Past_catches:_Blue_gill,_servings_eaten_numeric`,
#   carp_g_day,`Past_catches:_Common_Carp,_times_eaten_numeric`,`Past_catches:_Common_Carp,_servings_eaten_numeric`,
#   bass_g_day,`Past_catches:_Bass,_times_eaten_numeric`,`Past_catches:_Bass,_servings_eaten_numeric`,
#   crappie_g_day, `Past_catches:_Crappie,_times_eaten_numeric`,`Past_catches:_Crappie,_servings_eaten_numeric`,
#   tilapia_g_day,`Past_catches:_Tilapia,_times_eaten_numeric`,`Past_catches:_Tilapia,_servings_eaten_numeric`,
#   other_g_day,`Past_catches:_other,_times_eaten_numeric`,`Past_catches:_other,_servings_eaten_numeric`
#   )
# 
...
Alondra_cooking_methods <- cooking_methods_sub %>%
  ...
  Alondra_total_people <- length(unique(Alondra_cooking_methods$rowID))
...
for (lake_name in lakes) {
   ...
   income_counts <- consumer_x_income %>% 
    filter(updated_lake_name == lake_name)%>%# Filter for the current lake
    group_by(Annual_Household_Income) %>%  # Group by cooking method
    summarise(count_consumers = n_distinct(rowID), .groups = "drop")  # Count occurrences
  
  #calculate total number of consumers at the lake
    total_people_df <- consumer_x_income %>% filter(updated_lake_name == lake_name)
    total_people <- length(unique(total_people_df$rowID))
     total_people_list[[lake_name]] <- total_people  # Store total people for the lake
    
  #count total number of people exceeding threshold
     total_exceed_n <- consumer_x_income %>%
       filter(updated_lake_name == lake_name)%>%
       filter(exceed_threshold_indiv_lake == TRUE)%>%
     group_by(Annual_Household_Income) %>%  # Group by cooking method
    summarise(exceed_count = n_distinct(rowID), .groups = "drop")  # Count occurrences
       
    income_counts<-full_join(income_counts,total_exceed_n, by = "Annual_Household_Income")  
    income_counts <- income_counts %>%
      mutate(exceed_count = case_when(
      is.na(exceed_count) ~ "0",  # Replace NA with "0" (or other desired value)
      TRUE ~ as.character(exceed_count)  # Keep all other values as they are
  ))
    
dailyintake_x_income <- consumer_x_income %>%
...
income_counts <- income_counts %>%
  mutate(
    lake = lake_name,
    count_consumers = as.numeric(count_consumers),  # Ensure count is numeric
    exceed_count = as.numeric(exceed_count),  # Ensure exceed_count is numeric
    total_people = as.numeric(total_people),  # Ensure total_people is numeric
    percent_consumers = round((count_consumers / total_people) * 100, 0),  # Calculate percentage
    per_exceed = round((exceed_count / total_people) * 100, 0)  # Calculate exceed percentage
  )
...
# Combine results from all lakes into a single dataframe
income_counts_combined <- bind_rows(income_counts_list)

##Rearrange and simplify output 
income_counts_combined <- income_counts_combined %>%
  select(lake, Annual_Household_Income,count_consumers,percent_consumers,min,max,mean,median,per_exceed)
...
###Loop for number of people per ethnicity per lake
...
for (lake_name in lakes) {
   ...
   ethnicity_counts <- consumer_x_ethnicity %>% 
    filter(updated_lake_name == lake_name)%>%# Filter for the current lake
    group_by(Ethnicity) %>%  # Group by cooking method
    summarise(count_consumers = n_distinct(rowID), .groups = "drop")  # Count occurrences
  
  #calculate total number of consumers at the lake
    total_people_df <- consumer_x_ethnicity %>% filter(updated_lake_name == lake_name)
    total_people <- length(unique(total_people_df$rowID))
     total_people_list[[lake_name]] <- total_people  # Store total people for the lake
...
ethnicity_counts_combined <- ethnicity_counts_combined %>%
  select(lake, Ethnicity,count_consumers,percent_consumers,min,max,mean,median,per_exceed)
...
###Come back to QA
consumer_x_ethnicity<- fisherdb_consumers# %>% 
  mutate(Ethnicity = as.factor(Ethnicity))%>%
...
###household consumption from lakes surveyed and not surveyed
...
for (lake_name in names(lakes)) {
  ...
  ### Percent of consumers exceeding thresholds by lake (Exceed lake specific threshold by consuming fish ONLY at that lake)
      ...
      per_exceed_thresh_hh <- hh_consumers_species_long_adv %>%
        ...
        ungroup() %>%
        filter(!is.na(any_hh_exceeded_adv)) %>% #filters out NAs so that only TRUE / FALSE values are being assessed ##CHECK NAs
...
hh_lakes_summary_table_export <- hh_lakes_summary_table_export %>%
  select(county,lakes, everything())
