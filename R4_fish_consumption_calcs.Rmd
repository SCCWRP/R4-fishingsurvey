...
all(check_consistency$n == 0) #returns true if all groups are consisten
 ...
 ##histogram of age/gender of fishers who consume fish ##39 unique individuals ##CHECK why missing 4
...
# The total is lower here because: 
...
# 45 said they planned to eat the fish
# 12 missed the question
...
no_reason <- no_reason %>%
  #filter(if_all(ends_with("_why"), is.na)) %>%
  mutate(Combined_Responses = apply(
    select(., ends_with("_why")), 
    1, 
    function(row) {
      # Combine responses, remove NAs and "NA"
      combined = paste(row[!is.na(row) & row != "NA"], collapse = ", ")
      # Remove duplicates by splitting, deduplicating, and trimming
      unique_responses = unique(str_trim(str_split(combined, ",\\s*")[[1]]))
      paste(unique_responses, collapse = ", ")
    }
  ))%>%
  filter(is.na(Combined_Responses) | Combined_Responses == "" )

response_summary <- no_reason %>%
  summarise(
    with_response = sum(!is.na(Combined_Responses) & Combined_Responses != ""),
    blank_response = sum(is.na(Combined_Responses) | Combined_Responses == "")
  )

write.csv(no_reason, "no_reason.csv", row.names = FALSE)
...
income_counts_combined <- income_counts_combined %>%
  select(lake, Annual_Household_Income,count_consumers,percent_consumers,min,max,mean,median,per_exceed, count_exceed)
...
for (lake_name in names(lakes)) {  # Use names of the lakes list
  selected_lakes <- lakes[[lake_name]]  # Extract the lake(s) for the current iteration
   ...
   ethnicity_counts <- consumer_x_ethnicity %>% 
    filter(updated_lake_name %in% lake_name)%>%# Filter for the current lake
  ...
  #calculate total number of consumers at the lake
    total_people_df <- consumer_x_ethnicity %>% filter(updated_lake_name %in% lake_name)
    total_people <- length(unique(total_people_df$rowID))
     total_people_list[[lake_name]] <- total_people  # Store total people for the lake
...
ethnicity_counts_combined <- ethnicity_counts_combined %>%
  select(lake, Ethnicity,count_consumers,percent_consumers,min,max,mean,median,per_exceed, count_exceed)
