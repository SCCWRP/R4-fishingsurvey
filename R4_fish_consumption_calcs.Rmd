...
for (lake_name in names(lakes)) {
  ...
  per_exceed_thresh <- consumers_long %>%
    ...
    summarise(any_exceeded = any(exceed_threshold)) %>%
    ungroup() %>%
...
household_consumers_long_subset <- household_consumers_long %>%
  select(Number_in_household:cooking_details)%>%
  select(-(`If_State_addressed_concerns,_would_you_eat_fish?`:Interview_Comments))%>%
  select(fisherID, Number_in_household, Number_in_household_eating_caught_fish_including_fisher, household_member, Relation_to_fisher, age, gender, species_consumed,parts_eaten, cooking_details,  daily_intake, everything())
...
# Keep the rows that do not meet the condition as they are
non_joined_data <- hh_consumers_species_long %>%
  filter(!(hh_daily_intake_per_fish_per_lake > 0 & is.na(column)))
  
# Combine the two data frames back together
hh_consumers_species_long <- bind_rows(joined_data, non_joined_data) 
...
##During data clean up - check NAs here. Explore why.
##Need to check code to make sure things are filtering & calculating as expected
write.csv(hh_consumers_long, "hh_consumers_long.csv", , row.names = FALSE)
