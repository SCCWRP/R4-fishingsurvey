...
consumers_long <- consumers %>%
  ...
  select(rowID, Age_numeric, Observed_Gender, Catches_Info, Lakes, starts_with("Past_catches:") & ends_with(",_times_eaten_numeric"), starts_with("Past_catches:") & ends_with(",_amount_eaten_numeric"),
        starts_with("Past_catches:") & ends_with("parts_eaten"),`Past_catches:_Other,_list_species`, Annual_Household_Income, Number_in_household, Ethnicity)  %>%
...
##Create datset to use for parts of fish consumed analysis
...
##Dataframe "parts_consumed" is created from "consumers_long" in the previous code chunk 
...
##histogram of age/gender of fishers who consume fish
fish_consumers <- consumers_long %>%
  group_by(rowID) %>%
  summarise(Age_numeric = first(Age_numeric), Observed_Gender = first(Observed_Gender)) %>%
  ungroup()

fish_consumers$Observed_Gender <- factor (fish_consumers$Observed_Gender, levels = c("Female", "Male"))
