...
Surveyed_lakes_caught_fish <-caught_fish_long %>%
  filter(Lakes == "Alondra park"|
         Lakes == "Magic Johnson Lakes"|
         Lakes == "Legg lake"|
         Lakes == "Peck Road")
length(unique(Surveyed_lakes_caught_fish$rowID)) # 90 unique fishers
...
plot_summary_caught_fish <-summary_caught_fish %>%
  filter (lake_name != "All")

    ggplot(plot_summary_caught_fish, aes(x = lake_name, y = percent, fill = Catches_Info)) +
  geom_bar(stat = "identity", color = "black") +  # Use "identity" to stack based on count
  labs(
    title = "Fish Caught by Lake and Species",
    x = "Lake Name",
    y = "Count of Fish Caught",
    fill = "Fish Species"
  ) +
