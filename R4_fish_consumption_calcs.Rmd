...
```{r}
###address missing points & then rerun (currently not seeing sig trend)
...
exceed_thresh_income_counts<- consumers_long %>%
  ...
  group_by(Annual_Household_Income)%>%
  summarise(exceed_thresh_count = sum(exceed_threshold_avg_lake == TRUE, na.rm = TRUE)) %>%
  ungroup()
...
income_counts <- income_counts %>%
  ...
  mutate(percent_consumers = round((consumer_count/fisher_count)*100))%>%
  mutate(percent_exceed_ofconsumers = round((exceed_thresh_count/consumer_count)*100))%>%
  mutate(percent_exceed_offishers = round((exceed_thresh_count/fisher_count)*100))%>%
  mutate(percent_total = 100)

income_counts_export <- income_counts %>%
  select(Annual_Household_Income,fisher_count, percent_consumers, consumer_count, percent_exceed_ofconsumers, exceed_thresh_count, n, min, max, mean, median)
...
```
##Fish consumption by race
```{r}
###Look into how to test / interpret differences while accounting for relative number of people in each group

consumers_x_race <- consumers_long %>%
group_by(rowID, Ethnicity)%>%
    summarize(
    total_daily_intake = sum(daily_intake, na.rm = TRUE),
    exceed_threshold_avg_lake = any(exceed_threshold_avg_lake == TRUE, na.rm = TRUE)  # Handle TRUE values
  ) %>%
  ungroup()%>%
filter(!Ethnicity %in% c("No answer","Unknown", "Other/Unknown", "NA"))%>%
  mutate(Ethnicity = as.factor(Ethnicity))

ggplot(consumers_x_race, aes(x=Ethnicity, y =(total_daily_intake)))+
  geom_point(position="jitter", aes(color=exceed_threshold_avg_lake))+
   geom_boxplot(aes(fill = Ethnicity), alpha = 0.1, outlier.shape = NA) +  # Transparent boxplot with no outliers
   scale_y_break(c(500, 2100))+
  theme_minimal() +
  theme(legend.position = "none")

##Ethnicity consumers vs. nonconsumers
...
fisherdb_consumers <- fisherdb %>% mutate(consumer =(
  ...
  mutate(Ethnicity = as.factor(Ethnicity))%>%
  filter(!Ethnicity %in% c("No answer","Unknown", "Other/Unknown", "NA", "No Answer", "Prefer not to say"))%>%
  mutate(Ethnicity = replace(Ethnicity, Ethnicity == "white", "White")
  )

ethnicity_counts <- fisherdb_consumers %>%
  group_by(Ethnicity) %>%
  summarise(fisher_count = n()) %>%
  ungroup()

consumer_ethnicity_counts <- fisherdb_consumers %>%
  filter(consumer ==TRUE)%>%
  group_by(Ethnicity)%>%
  summarise(consumer_count = n()) %>%
  ungroup()
...
ethnicity_counts <- ethnicity_counts %>%
  ungroup()%>%
  filter(!is.na(consumer_count))%>%
  mutate(percent_consumers = (consumer_count/fisher_count)*100)%>%
  mutate(percent_nonconsumers = 100 - percent_consumers)%>%
  mutate(percent_exceed_ofconsumers = (exceed_thresh_count/consumer_count)*100)%>%
  mutate(percent_exceed_offishers = (exceed_thresh_count/fisher_count)*100)%>%
  mutate(percent_total = 100)

##Plot consumer counts as portion of ethnicity
# Assuming ethnicity_counts now has both ethnicity total counts and consumer counts
ggplot(ethnicity_counts, aes(x = Ethnicity)) +
  geom_bar(aes(y = fisher_count, fill = "Total Count"), stat = "identity") +  # Plot total counts
  geom_bar(aes(y = consumer_count, fill = "Consumer Count"), stat = "identity") +  # Overlay consumer counts
  scale_fill_manual(values = c("Total Count" = "gray", "Consumer Count" = "blue")) +  # Customize fill colors
  labs(y = "Count", x = "Ethnicity", title = "Consumer Counts as Portion of Ethnicity") +  # Customize labels
  theme_minimal()  # Clean theme

##Plot 

ethnicity_counts <- ethnicity_counts %>% 
   mutate(Ethnicity = factor(Ethnicity, levels = c("Black or African American",
                                                  "American Indian or Alaska Native", 
                                                  "Asian or Pacific Islander", 
                                                  "Hispanic or Latino", 
                                                  "White")))

# perc_consumers<-ggplot(ethnicity_counts, aes(x = Ethnicity)) +
#   geom_bar(aes(y = percent_total , fill= "Total Count"), stat = "identity") +  # Plot total counts
#   geom_bar(aes(y = percent_consumers, fill = "Consumer Count"), stat = "identity") +  # Overlay consumer counts
#   scale_fill_manual(values = c("Total Count" = "gray", "Consumer Count" = "#2786A0")) +  # Customize fill colors
#   labs(y = "Percent of fishers", x="", title = "Percent of fishers consuming fish") +  # Customize labels
#   theme_minimal() +
#   theme(legend.position = "bottom",
#         panel.grid.major = element_blank(), 
#         panel.grid.minor = element_blank()) # Clean theme
# 
# 
# per_exceed_thresh<- ggplot(ethnicity_counts, aes(x = Ethnicity)) +
#   geom_bar(aes(y = percent_total , fill= "Total Count"), stat = "identity") +  # Plot total counts
#   geom_bar(aes(y = percent_exceed_ofconsumers, fill = "Exceed"), stat = "identity") +  # Overlay consumer counts
#   scale_fill_manual(values = c("Total Count" = "gray", "Exceed" = "#D07C42")) +  # Customize fill colors
#   labs(y = "Percent of consumers", x = "", title = "Percent of consumers exceeding advisory") +  # Customize labels
#   theme_minimal() +
#   theme(legend.position = "bottom",
#         panel.grid.major = element_blank(), 
#         panel.grid.minor = element_blank())  # Clean theme
...
perc_all_eth<-ggplot(ethnicity_counts, aes(x = Ethnicity)) +
  geom_bar(aes(y = percent_total , fill= "Total Count"), stat = "identity") +  # Plot total counts
  geom_bar(aes(y = percent_consumers, fill = "Consumer Count"), stat = "identity") + # Overlay consumer counts
  geom_bar(aes(y = percent_exceed_offishers, fill = "Exceed"), stat = "identity") +  # Overlay consumer count
  scale_fill_manual(values = c("Total Count" = "gray", "Consumer Count" = "#2786A0","Exceed" = "#D07C42" )) +  # Customize fill colors
  labs(y = "Percent of fishers", x="", title = "Percent of fishers consuming fish") +  # Customize labels
  theme_minimal() +
  theme(legend.position = "bottom",
        panel.grid.major = element_blank(), 
...
# plot_ethnicity <- perc_consumers + per_exceed_thresh
# plot_ethnicity
...
ggsave(
  filename = "consumption_x_ethnicity.svg",
  plot = perc_all_eth, width = plot_width, height = plot_height, units = "px", dpi = 300
)
