...
consumers_long <- consumers %>%
  ...
  select(rowID, Age, Observed_Gender, Catches_Info, Lakes, starts_with("Past_catches:") & ends_with(",_times_eaten"), starts_with("Past_catches:") & ends_with(",_amount_eaten"),
        starts_with("Past_catches:") & ends_with("parts_eaten") ) %>%
...
# ###Test
#  lake_name <- "Legg lake"
#  age_group <- c("threshold_g_day_men18","threshold_g_day_child","threshold_g_day_women18_49","threshold_g_day_women50")
#  fish_name <- "Catfish"
...
### Percent of consumers exceeding thresholds by lake (Exceed lake specific threshold by consuming fish ONLY at that lake)
  per_exceed_thresh <- consumers_long %>%
    ...
    summarise(percent_exceed_lake = round((sum(any_exceeded_lake) / n()) * 100, 1)) %>%  #Summarizes entire column to      calculate percent true exceeding individual lake thresholds
